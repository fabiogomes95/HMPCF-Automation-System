...
# ==============================================================================
# 🚀 HACK DE COMPATIBILIDADE PARA LINUX (CACHYOS / WAYLAND)
# ==============================================================================
import eel
import eel.__init__ as eel_mod

# Mantemos os teus hacks intactos para funcionar no teu Linux
...
# Proteção para o Robô (Modo Casa)
try:
    import executor_rpa
    ...
    print(f"⚠️ MODO CASA: Automação gráfica desativada (Erro: {e})")

# Inicializa o Eel
eel.init('web')

# ==============================================================================
# 🩺 MÓDULO TRIAGEM (Ajustado para o teu robô atual)
# ==============================================================================

@eel.expose
def rodar_limpador(data_lote, enfermeiros_str):
    """
    Limpa e fatia em lotes de 99 para múltiplos enfermeiros.
    Gera o formato: PROFISSIONAL: NOME | DATA: XX/XX/XXXX
    """
    # 1. Corre a limpeza (certifica-te que o cpf_sus.py gera o pacientes.txt)
    ...
    # 2. Fatiamento e criação dos cabeçalhos no formato do teu executor_rpa.py
    for i in range(0, len(docs), chunk_size):
        chunk = docs[i:i+chunk_size]
        prof_atual = profs[idx_p % len(profs)]
        idx_p += 1
        
        # Formato exato que o teu split("|") espera:
        resultado_final.append(f"PROFISSIONAL: {prof_atual} | DATA: {data_lote}")
        resultado_final.extend(chunk)
        resultado_final.append("") # Linha em branco entre lotes
...
# ==============================================================================
# (MANTÉM TODAS AS OUTRAS FUNÇÕES DO TEU FICHEIRO ORIGINAL ABAIXO)
# ==============================================================================
...
def listar_producoes():
    ...
    nomes = [os.path.basename(a) for a in arquivos]; nomes.sort(reverse=True)
    return nomes
...
def preparar_rpa(nome_arquivo):
    ...
    # Se estiver no Linux e o ficheiro existir, ele tenta carregar os lotes
...
def buscar_pacientes_fb(termo):
    ...
    try:
        ...
        con.close(); return sorted(res, key=lambda x: x['nome'])[:50]
    except Exception:
        return [{'nome': 'FÁBIO GOMES DA SILVA', 'sus': '707802947623117', 'cpf': '13797250460', 'dtnasc': '10/05/1995'}]
